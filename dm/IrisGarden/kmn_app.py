...
norm_garden = norm_iris_garden
class_arr = iris_class_arr
...
def k_means(seeds):
    ...
    # class_arr = np.concatenate([c0[0], c1[0], c2[0]])
    # class_arr[0:len(c0)] = 1
    # class_arr[len(c0):(len(c0)+len(c1))] = 2
    # class_arr[(len(c0)+len(c1)):] = 3
    #
    # c_sepal_length = np.concatenate([c0[0], c1[0], c2[0]])
    # c_petal_width = np.concatenate([c0[2], c1[2], c2[2]])

    na0 = np.array(c0)
    nz0 = np.full((len(c0), 1), 0, dtype=int)
    na1 = np.array(c1)
    nz1 = np.full((len(c1), 1), 1, dtype=int)
    na2 = np.array(c2)
    nz2 = np.full((len(c2), 1), 2, dtype=int)
    nac0 = np.append(na0, nz0, axis=1)
    nac1 = np.append(na1, nz1, axis=1)
    nac2 = np.append(na2, nz2, axis=1)

    nac = np.append(nac0, nac1, axis=0)
    nac = np.append(nac, nac2, axis=0)

    print nac.shape
    print nac[0]
    print nac[149]

    c_sepal_length = nac[:, 0]
    c_petal_width = nac[:, 2]
    c_marker = nac[:, 4]
    ...
    plt.scatter(c_sepal_length, c_petal_width, c=['r', 'g', 'b'], cmap=c_marker)
...
# print get_centroid([[1, 1, 1, 1], [5, 5, 5, 5], [3, 3, 2, 4]])
